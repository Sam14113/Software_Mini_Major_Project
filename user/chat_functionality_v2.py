...
def create_group(user_ID, org_ID, group_name, participant_IDs, sender_rsa_sk):
    ...
    group_ID = communicate.claim_available_group_ID(org_ID)
    ...
    participants_alert_message_ID = auxiliary.get_new_random_ID([invitation_message_ID])
    ...
    recipient_IDs = [ID for ID in participant_IDs if ID != user_ID] # So that the user doesn't receive the alerts
    user.send.send_message(org_ID, recipient_IDs, sender_rsa_sk, invitation)
    user.send.send_message(org_ID, recipient_IDs, sender_rsa_sk, participants_alert)

    return group
def create_chat(user_ID, org_ID, group, chat_name, sender_rsa_sk):
    # Creating chat

    chat_ID = auxiliary.get_new_random_ID([chat.ID for chat in group.chats])
